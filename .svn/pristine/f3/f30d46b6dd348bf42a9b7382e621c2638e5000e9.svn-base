...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* find register */
	const reg_t *regtmp = find_register(str);
	if (!regtmp) {
...
parse_t parse_symbol(char **ptr, char **name)
{
	...
	char *cur = symbol;
	while (!IS_BLANK(str) && (IS_LETTER(str) || (cur != symbol && IS_DIGIT(str)))) {
		*cur++ = *str++;
	}
	*cur = '\0';
    /* set 'ptr' and 'name' */
...
parse_t parse_digit(char **ptr, long *value)
{
	...
	int base;
	if (*str == '0' && *(str + 1) == 'x') { /* errprone */
		base = 16;
	} else {
		base = 10;
	}
	long vtmp = sign * strtoull(str, &str, base);
    /* set 'ptr' and 'value' */
	*ptr = str;
	*value = vtmp;
    return PARSE_DIGIT;
...
*           call SUM  #invoke SUM function */
	...
	if (IS_LETTER(str) || *str == '.') {
		...
		switch (HIGH(inst->code)) {
			...
			case I_RRMOVQ:
			case I_ALU:
				if (parse_reg(&str, &rA) == PARSE_ERR ||
						parse_delim(&str, ',') == PARSE_ERR ||
						parse_reg(&str, &rB) == PARSE_ERR) {
					line->type = TYPE_ERR;
					return line->type;
				}
			...
			case I_RMMOVQ:
				if (parse_reg(&str, &rA) == PARSE_ERR ||
						parse_delim(&str, ',') == PARSE_ERR ||
						parse_mem(&str, &value, &rB) == PARSE_ERR) {
					line->type = TYPE_ERR;
					return line->type;
				}
				bin.codes[1] = HPACK(rA, rB);
				...
				break;
			case I_MRMOVQ:
				if (parse_mem(&str, &value, &rB) == PARSE_ERR ||
						parse_delim(&str, ',') == PARSE_ERR ||
						parse_reg(&str, &rA)) {
					line->type = TYPE_ERR;
					return line->type;
				...
				case D_DATA: 
					{
						...
						if (pt == PARSE_DIGIT) {
							set_n_val(line->y64bin.codes, value, bytes);
						} else if (pt == PARSE_SYMBOL) {
							add_reloc(name, &(line->y64bin));
