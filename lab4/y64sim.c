...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
		case I_RRMOVQ: case I_RMMOVQ: case I_MRMOVQ: case I_ALU:
		case I_PUSHQ: case I_POPQ: case I_IRMOVQ:
			{
				...
				if ((!NORM_REG(regA) && !NONE_REG(regA)) ||
					(!NORM_REG(regB) && !NONE_REG(regB)) ||
					(icode == I_IRMOVQ && !NONE_REG(regA)) ||
					((icode == I_PUSHQ || icode == I_POPQ) && !NONE_REG(regB))) {
					err_print("PC = 0x%lx, Invalid register id", sim->pc); /* to be modified */
					return STAT_INS;
				}
    ...
    switch (icode) {
		case I_RMMOVQ: case I_MRMOVQ: case I_IRMOVQ: case I_JMP: case I_CALL:
    ...
    switch (icode) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
		{
			long_t val = get_reg_val(sim->r, regA);
			long_t pos = get_reg_val(sim->r, regB);
			set_long_val(sim->m, pos + imm, val); /* to be modified */
			sim->pc = next_pc;
			break;
		}
      case I_MRMOVQ: /* 5:0 regB:regA imm */
		{
			long_t pos = get_reg_val(sim->r, regB);
			long_t val;
			if (!get_long_val(sim->m, pos + imm, &val)) {
				err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, pos + imm);
				return STAT_ADR;
			}
      ...
      case I_ALU: /* 6:x regA:regB */
		{
			...
			long_t result = compute_alu(ifun, valA, valB);
			set_reg_val(sim->r, regB, result);
			sim->cc = compute_cc(ifun, valA, valB, result);
			sim->pc = next_pc;
			break;
		}
      case I_JMP: /* 7:x imm */
      ...
      case I_CALL: /* 8:x imm */
		{
			long_t sp = get_reg_val(sim->r, REG_RSP);
			sp -= 8;
			set_reg_val(sim->r, REG_RSP, sp);
			if (!set_long_val(sim->m, sp, next_pc)) {
				err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, sp);
				return STAT_ADR;
			}
			sim->pc = imm;
			break;
      ...
      case I_RET: /* 9:0 */
		{
			long_t ret_addr;
			long_t sp = get_reg_val(sim->r, REG_RSP);
			get_long_val(sim->m, sp, &ret_addr);
			sp += 8;
			set_reg_val(sim->r, REG_RSP, sp);
			sim->pc = ret_addr;
			break;
